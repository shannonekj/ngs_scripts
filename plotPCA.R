...
# Plot
PC_matrix <- as.matrix(eig$vectors)
PC <- as.data.frame(PC_matrix)

colnames(PC) <- gsub("V", "PC", colnames(PC))
PC$Pop <- factor(annot$YEAR)
PC$Batch <- factor(annot$BATCH)
PC$Well <- factor(annot$WELL)
PC$Ind <- factor(annot$IND)
...
#ggplot() + geom_point(data=PC, aes_string(x=x_axis, y=y_axis, color="Pop")) + geom_text(data=PC, aes_string(x=x_axis, y=y_axis, label="Lab", vjust= -0.784), size = 2)+ ggtitle(title)
